## Local Vendoring Note

This copy of `tauri-plugin-sql` is intentionally vendored for this project.

Why:

- RxDB's Tauri SQLite adapter issues explicit `BEGIN` / `COMMIT` transactions.
- Upstream `tauri-plugin-sql` uses a SQLite `sqlx::Pool`, which allowed those statements to land on different connections.
- In this project that caused native Tauri RxDB writes to fail with errors like `cannot commit - no transaction is active`.

Local patch:

- File: `src/wrapper.rs`
- Change: SQLite connections use `SqlitePoolOptions::new().max_connections(1)` instead of `Pool::connect(...)`.

Project wiring:

- `apps/web/src-tauri/Cargo.toml` points `tauri-plugin-sql` at this vendored path dependency.

Removal conditions:

- Remove this vendor copy only after upstream `tauri-plugin-sql` provides a SQLite transaction-safe connection strategy compatible with RxDB, and the project has been switched back to the upstream dependency.
